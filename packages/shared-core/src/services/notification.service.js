...
const admin_js_1 = require("../firebase/admin.js");
...
const devicePushTokens = new Map();
class NotificationService {
        ...
        if (params.sendPush !== false) {
            ...
            if (device && device.token) {
                try {
                    ...
                    console.warn(`⚠️ [PUSH-ERROR] Failed to send push popup:`, err.message);
                }
            }
        }
     ...
     */
    static async dispatchPhonePushNotification(payload) {
        // 1. If Firebase Admin SDK is initialized and token is a native FCM device registration token
        const messaging = (0, admin_js_1.getFirebaseMessaging)();
